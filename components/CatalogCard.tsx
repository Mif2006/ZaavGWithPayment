...
interface CatalogCardProps {
  ...
  onAddToCart: (item: CatalogItem, e: React.MouseEvent) => void;
  onLearnMore: (item: CatalogItem, e: React.MouseEvent) => void;
...
const CatalogCard: React.FC<CatalogCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="bg-black/40 dark:bg-black/40 backdrop-blur-xl border border-white/20 dark:border-white/20 
                 rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden group cursor-pointer 
                 transition-all duration-300 hover:scale-[1.02] hover:border-purple-400/60 
    ...
    >
      ...
      <div className="p-4 h-full to-black/30 dark:from-black/50 dark:to-black/30 backdrop-blur-sm">
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-sm font-serif text-white dark:text-white line-clamp-2 font-medium">
            {item.name}
          </h3>
        ...
        <div className="flex space-x-2">
          ...
          <motion.button
            ...
            disabled={
              ...
              Object.values(item.sizes).every((qty) => qty === 0)
            }
